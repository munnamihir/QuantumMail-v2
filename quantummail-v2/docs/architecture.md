...
* requester details
* approved\_org\_id + approved\_admin\_user\_id
* email delivery status fields (email\_sent\_at, email\_last\_error, email\_last\_type)

#### `qm_setup_tokens`

Stores one-time setup tokens (hash only) + OTP verification fields:

* token\_hash = sha256(token) (raw token never stored)
* email verification:
  * otp\_hash (HMAC-SHA256 with TOKEN\_SECRET)
  * otp\_expires\_at, otp\_sent\_at
  * otp\_attempts, otp\_last\_attempt\_at
* context:
  * email, org\_name, admin\_username
...
* `requireSuperAdmin` orgId == PLATFORM\_ORG\_ID and role == SuperAdmin
...
GET /public/setup-admin-info?orgId\&token
...
POST /auth/setup-admin/send-code { orgId, token }

* generates 6-digit OTP
* stores `otp_hash = HMAC(code)` with TOKEN\_SECRET
...
POST /auth/setup-admin/verify-code { orgId, token, code }
...
* `iv`, `ciphertext`, `wrappedKeys` (map userId -> wrappedDEK) Optional:
* `aad`
...
* `GET /admin/alerts?minutes=60`
  * detects:
    * login\_failed (high)
    * decrypt\_denied (critical)
    * clear\_user\_pubkey (medium)
...
`GET /admin/analytics?days=7&staleKeyDays=90` Returns:
...
* hashed OTP using HMAC with TOKEN\_SECRET (not reversible)
