...
QuantumMail implements client-side envelope encryption with per-recipient key wrapping and organization-bound decryption. All encryption and decryption operations occur only on the user’s device.
...
{% stepper %}
{% step %}
User logs into their organization
{% endstep %}

{% step %}
User selects email content and optional attachments
{% endstep %}

{% step %}
Client generates a Data Encryption Key (DEK)
{% endstep %}

{% step %}
Email content encrypted locally
{% endstep %}

{% step %}
Attachments encrypted locally
{% endstep %}

{% step %}
DEK wrapped individually using each recipient’s public key
{% endstep %}
...
{% step %}
Secure access link inserted into the email
{% endstep %}
{% endstepper %}
...
{% step %}
Recipient logs into the same organization
{% endstep %}

{% step %}
Client retrieves wrapped DEK
{% endstep %}

{% step %}
DEK unwrapped using recipient’s private key
{% endstep %}
...
Encryption access is governed via the following API endpoints: /auth/login\
/org/register-key\
/org/users\
/api/messages

Only users with registered public keys inside the same organization and included in the recipient list can decrypt messages.
