...
async function hashPassword(pw) {
  return await bcrypt.hash(String(pw || ""), SALT_ROUNDS)
}
...
app.get("/api/messages/:id", requireAuth, async (req, res) => {
  try {
   ...
   /* =========================
      🔐 RECOVERY MODE — Admin only
   ========================= */

   // Only admins can use recovery mode
   const requestingUser = org.users?.[req.qm.user.userId];
   if (!requestingUser || !["Admin", "SuperAdmin"].includes(requestingUser.role)) {
     return res.status(403).json({ error: "Recovery mode requires Admin role" });
   }

   // Must have an approved recovery request in the database
   const { rows: recoveryRows } = await pool.query(
     `SELECT * FROM qm_recovery_requests 
   ...
   if (!recoveryRows.length) {
     return res.status(403).json({ error: "No approved recovery request found" });
   }
