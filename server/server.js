...
app.get("/api/recovery/pending", requireAuth, async (req, res) => {
  const userId = req.qm.user.userId;
...
app.post("/api/recovery/approve", requireAuth, async (req, res) => {
  const userId = req.qm.user.userId;
  const deviceId = req.headers["x-qm-device-id"];
  const { request_id, encrypted_private_key } = req.body;

  const { rows: reqRows } = await pool.query(`
    SELECT requester_device_id FROM qm_recovery_requests
  ...
  if (reqRows[0].requester_device_id === deviceId) {
  ...
  const { rows } = await pool.query(`
    SELECT COUNT(*) as count FROM qm_recovery_approvals WHERE request_id=$1
  `, [request_id]);
...
app.get("/api/recovery/finish/:id", requireAuth, async (req, res) => {
  const { id } = req.params;
  const userId = req.qm.user.userId;

  const { rows } = await pool.query(`
    SELECT status FROM qm_recovery_requests
    WHERE request_id=$1 AND user_id=$2
  `, [id, userId]);
  ...
  const { rows: approvals } = await pool.query(`
    SELECT sig_b64 FROM qm_recovery_approvals
    WHERE request_id=$1
