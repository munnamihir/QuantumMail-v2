...
deviceRoutes.post("/register", requireAuth, async (req, res) => {
  try {
    ...
    const { device_id, label, device_type = "desktop", pub_jwk } = req.body || {};
    ...
    const { rows } = await pool.query(
      `insert into qm_devices
       ...
       values ($1,$2,$3,$4,$5::jsonb,'active',now(),now(),false)
       on conflict (user_id, device_id) do update set
         ...
         revoked = false
       returning *`,
      [userId, device_id, label || "", device_type, JSON.stringify(pub_jwk)]
    );
