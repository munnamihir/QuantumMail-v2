...
document.getElementById("startRecovery").onclick = () => {
  chrome.runtime.sendMessage(
    { type: "QM_START_RECOVERY" },
    (res) => {
      if (res.ok) {
        msgEl.innerText = "Started: " + res.payload.request_id;
      } else {
...
document.getElementById("loadPending").onclick = () => {
  chrome.runtime.sendMessage(
    { type: "QM_LOAD_RECOVERY_PENDING" },
    (res) => {
      if (!res.ok) return;

      pendingList.innerHTML = "";

      res.payload.pending.forEach(r => {
        const div = document.createElement("div");

        div.innerHTML = `
          <div style="margin:6px;padding:6px;border:1px solid #ccc;">
            ${r.request_id}
            <button data-id="${r.request_id}">Approve</button>
          </div>
        `;

        div.querySelector("button").onclick = () => {
          approveRecovery(r.request_id);
        };
...
function approveRecovery(requestId) {
  chrome.runtime.sendMessage(
    {
      type: "QM_APPROVE_RECOVERY",
      requestId
    },
    (res) => {
      msgEl.innerText = JSON.stringify(res.payload);
    }
  );
}
