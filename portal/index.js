...
function computeSignupUI() {
  ...
  const orgHelp = $("suOrgHelp");
  const summary = $("suSummary");
  ...
  if (signupType === "Individual") {
    // Individual can create org (no orgId required) OR join existing (orgId required)
    if (indivMode === "create") {
      if (orgRow) orgRow.style.display = "none";
      if (inviteWrap) inviteWrap.style.display = "none";
      if (wantAdminRow) wantAdminRow.style.display = "";
      if (orgHelp) orgHelp.textContent = "Org ID will be generated for you after signup.";
      if (summary) summary.textContent = "You’ll create a personal org. You can share your Org ID to let others join.";
    } else {
      if (orgRow) orgRow.style.display = "";
      if (inviteWrap) inviteWrap.style.display = "none";
      if (wantAdminRow) wantAdminRow.style.display = "none"; // joining existing -> role not chosen here
      if (orgHelp) orgHelp.textContent = "Enter the Org ID you want to join.";
      if (summary) summary.textContent = "You’ll join an existing org using Org ID (no one-time code).";
    }
    return;
  }

  // OrgType
  // Requires Org ID + Invite Code. Role is determined by invite.
  if (signupType === "OrgType") {
    ...
    if (orgHelp) orgHelp.textContent = "Enter the organization Org ID provided by the admin.";
    if (summary) summary.textContent = "You’ll join a company org using a single-use code generated by the admin.";
  }
}
